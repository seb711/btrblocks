  ...
  void operator()(INTEGER* dest,
    ...
    if (tuple_count >= w * 4) {
      while (i < tuple_count - (w * 4 - 1)) {
        // Load codes.
        svuint32_t codes_0 = svld1_u32(tp, reinterpret_cast<const unsigned int*>(codes + w));
        svuint32_t codes_1 = svld1_u32(tp, reinterpret_cast<const unsigned int*>(codes + 2 * w));
        svuint32_t codes_2 = svld1_u32(tp, reinterpret_cast<const unsigned int*>(codes + 3 * w));
        svuint32_t codes_3 = svld1_u32(tp, reinterpret_cast<const unsigned int*>(codes + 4 * w));
        ...
        // store values
        svst1_s32(tp, reinterpret_cast<int*>(dest + w), values_0);
        svst1_s32(tp, reinterpret_cast<int*>(dest + 2 * w), values_1);
        svst1_s32(tp, reinterpret_cast<int*>(dest + 3 * w), values_2);
        svst1_s32(tp, reinterpret_cast<int*>(dest + 4 * w), values_3);
